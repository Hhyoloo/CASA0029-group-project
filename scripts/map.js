...
Promise.all([
  fetch("data/bird_hotspot.geojson").then(res => res.json()),
  fetch("data/urban_condition_summary.json").then(res => res.json()),
  fetch("data/woodland_final.geojson").then(res => res.json()),
  fetch("data/water_final.geojson").then(res => res.json())
])
...
function addInteractiveMapPolish(layer) {
  if (!layer) return;
  layer.eachLayer(l => {
    l.on("mouseover", () => {
      l.setStyle({ weight: 2.2, fillOpacity: 0.95 });
      l.bringToFront();
    });
    l.on("mouseout", () => {
...
window.addEventListener("load", () => {
  if (geojsonLayer) addInteractiveMapPolish(geojsonLayer);
});
